...
from medusa.core.legacy.biosignals.eeg import montages


class MEEGChannel(SerializableComponent):
...
class EEGChannelSet(SerializableComponent):
    ...
    def set_standard_montage(self, l_cha=None, l_reference=None, l_ground=None,
        ...
        montage : str {'10-20'|'10-10'|'10-05'} or dict
            ...
            as montages.read_montage_file and pass the returned dict here.
        drop_landmarks : bool
        ...
        if isinstance(montage, str):
            ...
            montage = montages.get_standard_montage(standard=montage,
                                                    dim=self.dim,
        ...
        if l_reference is not None:
            if l_reference in montage:
                ...
                if allow_unlocated_channels:
                    ...
                    raise montages.ChannelNotFound(l_reference)
        ...
        for label in labels:
            ...
            if label in montage:
                ...
                if allow_unlocated_channels:
                    ...
                    raise montages.ChannelNotFound(label)
        ...
        if l_ground is not None:
            if l_ground in montage:
                ...
                if allow_unlocated_channels:
                    ...
                    raise montages.ChannelNotFound(l_ground)
            ground['label'] = ground
        else:
            ground = None
        # Check channels
...
class MEGChannelSet(SerializableComponent):
...
class EEG(Signal):
    ...
    def __init__(self, times, signal, fs, channel_set, **kwargs):
        ...
        # Modality-specific extra check
        if self.signal.shape[1] != channel_set.n_cha:
            raise ValueError(
                "Signal with shape [samples x channels] does not match "
                "with the number of channels in the channel set"
            )
    ...
    def from_serializable_obj(cls, dict_data):
        ...
        cs = dict_data.get('channel_set')
        if isinstance(cs, dict):
            dict_data['channel_set'] = EEGChannelSet.from_serializable_obj(cs)
