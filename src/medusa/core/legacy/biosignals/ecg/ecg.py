...
ECG_12LEADS = {
    ...
    'electrodes': {
        ...
        "RA": "Right Arm â€” Anywhere between the right shoulder and the wrist",
        "LA": "Left Arm â€” Anywhere between the left shoulder and the wrist",
        "RL": "Right Leg â€” Anywhere above the right ankle and below the torso (ground/reference)",
        "LL": "Left Leg â€” Anywhere above the left ankle and below the torso",
...
class ECGChannelSet(SerializableComponent):
...
class ECG(Signal):
    ...
    def __init__(self, times, signal, fs, channel_set, **kwargs):
        ...
        signal : list or numpy.ndarray
            2D numpy array [n_samples x n_channels]. ECG samples (the units
    ...
    @classmethod
    def from_serializable_obj(cls, dict_data):
        # Reconstruct the channel set if it was serialized as a dict
        cs = dict_data.get('channel_set')
        if isinstance(cs, dict):
            dict_data['channel_set'] = ECGChannelSet.from_serializable_obj(cs)
