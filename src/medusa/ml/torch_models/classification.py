...
* :class:`TorchClassifier` — backbone + **one** softmax head. Plain sklearn
  ...
  ``score(X, y)``; the head is sized from ``np.unique(y)`` at fit time.
...
class ClassificationTask(pl.LightningModule):
    ...
    def __init__(self, backbone: nn.Module, n_classes: int, lr: float = 1e-3):
        ...
        self.head = nn.Linear(backbone.backbone_features, n_classes)
        self.lr = lr
    ...
    def validation_step(self, batch, batch_idx):
        x, y = batch
        self.log('val_loss', F.cross_entropy(self(x), y), prog_bar=True)

    def configure_optimizers(self):
        """Adam with PyTorch defaults over all trainable parameters."""
...
class MultiTaskClassificationTask(pl.LightningModule):
    ...
    **different** datasets uniformly — the estimator feeds these via
    ...
    def __init__(self, backbone: nn.Module, heads: dict, lr: float = 1e-3):
        super().__init__()
        self.backbone = backbone
        self.heads = nn.ModuleDict({
            name: nn.Linear(backbone.backbone_features, n_classes)
            for name, n_classes in heads.items()
        })
        self.lr = lr
...
# ---------------------------------------------------------------------------- #
# sklearn estimators (public API). Constructor parameters are inherited from
# the engine (:class:`._engine._BaseTorchEstimator`) — no boilerplate __init__.
# ---------------------------------------------------------------------------- #
...
class TorchClassifier(ClassifierMixin, _BaseTorchEstimator):
    ...
    Examples
    --------
    >>> clf = TorchClassifier(EEGNet(...), max_epochs=50, val_split=0.2)
...
class TorchMultiTaskClassifier(_BaseTorchEstimator):
    ...
    * a dict ``{task_name: X_task}`` — each task trained on its **own**
    ...
    >>> mt = TorchMultiTaskClassifier(EEGNet(...), val_split=0.2)
    ...
    def fit(self, X, y: dict):
        ...
        for task_name, y_task in y.items():
            ...
            datasets[task_name] = TensorDataset(
                self._prepare(X_task), torch.as_tensor(y_idx))

        train, val = {}, {}
        for task_name, dataset in datasets.items():
            train[task_name], val_loader = self._loaders_from_dataset(dataset)
