...
def signal_orthogonalization_cpu(signal_1, signal_2):
    ...
    signal_ort : numpy 3D matrix
        MEEG ortogonalised signals. The first dimension is epochs. The second
         dimension is samples. The fourth dimension is the channel of first signal, and the second dimension is
        regarding which channel of second signal it has been orthogonalized the
        channel in third dimension. [n_epochs, n_samples, n_channels, n_channels].
...
def signal_orthogonalization_gpu_new(signal_1, signal_2):
    """
    STILL NOT WORKING
    ...
    signal_1_chan = tf.transpose(tf.reshape(tf.tile(signal_1, (1, n_chan_1, 1)),
                          (n_epo_1,  n_samp_1, n_chan_1 * n_chan_1)),perm=[0,2,1])
    ...
    signal_ort = tf.transpose(tf.reshape(signal_2_chan - beta *
                                         signal_1_chan,(n_epo_1,n_chan_2,
                                                        n_chan_1,n_samp_1)),
                              perm =(0,3,1,2)).numpy()
